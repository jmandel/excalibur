...
### Android Wasmtime preferred path implementation

- Implemented NDK/JNI Wasmtime as the Android app's preferred runtime path rather than a throwaway spike.
- Added `scripts/build_runtime_artifacts.py` as the shared programmatic runtime packaging pipeline. It translates the legacy-EH CPython artifact to exnref with Binaryen, writes the same `wasi/python.wasm` into web, consumer, and Android runtime zips, and can add an Android-only aarch64 `wasi/python-aarch64-android.cwasm` acceleration artifact.
- Confirmed ahead-of-time precompile is possible with Wasmtime CLI: `wasmtime compile -W exceptions=y --target aarch64-linux-android /tmp/python-exnref.wasm`, producing a target-specific ~62 MiB `.cwasm`. This can be bundled in the APK and deserialized on-device. It is an optimization artifact; semantic source of truth remains the shared exnref wasm and Python/calibre files.
- Added Android JNI/CMake bridge `libkindle_wasm_runtime.so` linking `libwasmtime.so` for arm64-v8a. The native bridge enables WebAssembly exceptions, configures WASI preview1, preopens the unpacked runtime and work directory, captures stdout/stderr, prefers the precompiled `.cwasm` when present, and falls back to compiling `wasi/python.wasm`.
